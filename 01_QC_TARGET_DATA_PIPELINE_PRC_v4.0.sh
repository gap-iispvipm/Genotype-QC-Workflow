...
# RENAME ORIGINAL FILES: original files needs to be renamed adding at the begining of the name "Target_data" (for .bed/.bim/.fam files).
...
# FOLDER MUST CONTAINING FILES:
	####01_QC_TARGET_DATA_PIPELINE_PRC_v4.0.sh
	####Target_data.bed
	####Target_data.bim
	####Target_data.fam
...
# In this script, THE ONLY variables that need to be modified are the followings: 
	...
	runplink="/home/pol/bin/PLINK/plink"
...
Step 6: PERFORMED = %s
Step 6: relatedness = %s" "$step0fam" "$step1" "$mymissigness1" "$mymissigness2" "$step2" "$step3" "$mymaf" "$step4" "$myhwe1" "$myhwe2" "$step5" "$mywindowsize" "$myshiftwindow" "$mypairwiser2" "$step6" "$myrelatedness" > 01_QC_TARGET_DATA_PIPELINE_PRC_v4.0.log
...
#0.1# Check if "rename" package is installed and if not, install it.
	required_pkg="rename"
	...
	if [ "" = "$pkg_OK" ]; 
	   then
	      ...
	      echo "# $required_pkg package already installed. No setting needed."
	fi
...
#0.2# FOR .fam file: 
	...
	if [ "$step0fam" = "YES" ];
	  then
		...
		for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/0A_/' $i ; done
		...
		for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/0B_/' $i ; done
...
#0.3# FOR .bim file: remove indels to work with snps.
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/0C_/' $i ; done
...
if [ "$step1" = "NO" ];
   then
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/1A_/' $i ; done
		# Generate plots to visualize the missingness results.
		$runplink --bfile Target_data --missing
		Rscript --no-save QC1_Hist_miss.R
		rename -f 's/^/1A_/' plink.* hist*
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/1B_/' $i ; done
		# Generate plots to visualize the missingness results.
		$runplink --bfile Target_data --missing
		Rscript --no-save QC1_Hist_miss.R
		rename -f 's/^/1B_/' plink.* hist*

	# Delete SNPs with missingness >0.02 ($mymissigness2).
	$runplink --bfile Target_data --geno $mymissigness2 --make-bed --out Target_data
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/1C_/' $i ; done
		# Generate plots to visualize the missingness results.
		$runplink --bfile Target_data --missing
		Rscript --no-save QC1_Hist_miss.R
		rename -f 's/^/1C_/' plink.* hist*
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/1D_/' $i ; done
		# Generate plots to visualize the missingness results.
		$runplink --bfile Target_data --missing
		Rscript --no-save QC1_Hist_miss.R
		rename -f 's/^/1D_/' plink.* hist*
...
if [ "$step2" = "NO" ];
   then
	...
	$runplink --bfile Target_data --check-sex
		...
		rename -f 's/^/1D_/' plink.* Gender_* Men_* Women* 
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/2_/' $i ; done
	# This command removes the list of individuals with the status PROBLEM.
		...
		# Generate plots to visualize the sex-check results.
		Rscript --no-save QC2_Gender_check.R
		rename -f 's/^/2_/' plink.* Gender_* Men_* Women*
...
if [ "$step5" = "NO" ];
   then
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/5_/' $i ; done
...
if [ "$step6" = "NO" ];
   then
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/6A_/' $i ; done
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/6B_/' $i ; done
	...
	for i in $( find . -name 'Target_data.*' -printf '%f\n' | awk '!/.bed/ && !/.bim/ && !/.fam/' ); do rename -f 's/^/6C_/' $i ; done
